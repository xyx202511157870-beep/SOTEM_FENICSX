...
def add_magnetic_stability_section(
    document: Document, result_dir: Path, profile: dict
) -> None:
    ...
    add_table(
        ...
        [
            (
                "模型",
                f"100 m 平行导线；{profile['size_text']} 渗流通道；埋深 20 m",
            ),
...
def _build_magnetic_only_report(result_dir: Path, output_path: Path) -> Path:
    ...
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = title.add_run(
        f"100 m 平行导线 {profile['size_text']} 渗流通道磁场稳定性报告"
    )
    ...
    add_magnetic_stability_section(document, result_dir, profile)
