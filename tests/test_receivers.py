...
def test_point_receiver_samples_hj_electric_faces_and_magnetic_edges():
    ...
    np.testing.assert_allclose(ex_receiver.sample_hj(mesh, e_faces, h_edges), expected_ex[0])
    np.testing.assert_allclose(hz_receiver.sample_hj(mesh, e_faces, h_edges), expected_hz[0])
    np.testing.assert_allclose(bz_receiver.sample_hj(mesh, e_faces, h_edges, mu=4.0), 4.0 * expected_hz[0])
...
def test_point_receiver_samples_hj_db_dt_from_edge_h_states():
    ...
    value = receiver.sample_hj_time_derivative(mesh, e_faces, h_new, h_old, dt=2.0, mu=4.0)
