...
| 论文模型 metadata/曲线产物框架 | 已提取 Song et al. 2025 模型参数；Fig. 2/3/7/8/12/15 的 PDF 页码、单位、面板、图注和裁剪框已写入 digitization target；`published-paper-figure-pages --crop-figures` 可从本地 PDF 渲染目标页和单图 PNG；能从数字化 CSV 写出 `paper_response_overlay.png`, `paper_relative_error_curves.png`, `runtime_diagnostics.json` | 真实曲线数值仍需数字化或获得表格数据 |
